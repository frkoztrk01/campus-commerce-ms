...
exports.renderAdminLogsHtml = renderAdminLogsHtml;
function renderAdminLogsHtml(entries) {
    const rows = entries
        .map((e) => `<tr><td>${escapeHtml(e.timestamp)}</td><td>${escapeHtml(e.method)}</td><td>${escapeHtml(e.path)}</td><td>${e.statusCode}</td><td>${e.durationMs}</td></tr>`)
        .join('');
    return `<!DOCTYPE html>
<html lang="en">
<head>
